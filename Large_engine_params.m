...
% Geometry
passage_pipe.geom.radius = 30e-2; % m
passage_pipe.geom.length = 60e-2; % m 
...
alternator.converter.efficiency = 0.8;
...
% Geometry (geom)
rhombic_drive.geom.gear_rad = 90e-2; % m
rhombic_drive.geom.gear_mass = 17.813; % kg
rhombic_drive.geom.crank_inertia = 0.5*rhombic_drive.geom.gear_mass*rhombic_drive.geom.gear_rad^2; % kg*m^2

% Drive paramters
rhombic_drive.drive.disp_link_length = 90e-2; % m
rhombic_drive.drive.power_link_length = 120e-2; % m 
rhombic_drive.drive.disp_crank_radius = 11.574e-2; % m
rhombic_drive.drive.power_crank_radius = 19.8529e-2; % m
...
% Geometry (geom)
displacer_piston.geom.radius = 30e-2; % m
displacer_piston.geom.cyl_inner_radius = 29.223e-2; % m
displacer_piston.geom.length = 180e-2; % m
displacer_piston.geom.wall_thickness = 9e-2; % m
displacer_piston.geom.clearance_vol = 8e-5; % m^3
...
power_piston = struct();
% Geometry (geom)
power_piston.geom.radius = 30e-2; % m
power_piston.geom.length = 180e-2; % m
power_piston.geom.clearance_vol = 8e-5; % m^3
...
%% Ambient
geometry.cooler.length = 90e-2; % m
...
A = -pi*(displacer_piston.geom.cyl_inner_radius^2 - displacer_piston.geom.radius^2); % Area m^2
...
deltaOm = 50;  % increase in angular speed (rad/s)
